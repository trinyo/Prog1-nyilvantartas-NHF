...
/* To include strdup and strndup functions (since c20) 
 * in debugmalloc administration mechanism. */
 
static inline char * debugmalloc_strdup (const char *s, char const *func, char const *expr, char const *file, unsigned line)
{
  size_t len = strlen (s) + 1;
  void *new = debugmalloc_malloc_full (len, func, expr, file, line, 0);
...
static inline char * debugmalloc_strndup (const char *s, size_t n, char const *func, char const *expr, char const *file, unsigned line)
{
  size_t len = strlen (s);
  ...
  void *new = debugmalloc_malloc_full (len+1, func, expr, file, line, 0);
...
#define strdup(S) debugmalloc_strdup((S), "strdup", #S, __FILE__, __LINE__)
#define strndup(S,N) debugmalloc_strndup((S), (N), "strndup", #S, __FILE__, __LINE__)
